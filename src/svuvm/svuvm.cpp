...
int vpi_mcd_printf_wrap(unsigned int mcd, char *format, py::args args) {
  std::vector<char> c_args;
  for (const auto &arg : args) {
    c_args.push_back(arg.cast<char>());
  }
  int result = vpi_mcd_printf(mcd, format, c_args.data());
  return result;
}

int vpi_printf_wrap(char *format, py::args args) {
  std::vector<char> c_args;
  for (const auto &arg : args) {
    c_args.push_back(arg.cast<char>());
  }
  int result = vpi_printf(format, c_args.data());
  return result;
}
...
PYBIND11_MODULE(svuvm, m) {
  ...
  py::class_<s_vpi_arrayvalue>(vpi, "VpiArrayValue")
      ...
      .def(py::init([](unsigned int format, unsigned int flags,
        ...
        return new s_vpi_arrayvalue{format, flags,
                                    {.reals = new double[reals.size()]}};
      }))
      .def(py::init([](unsigned int format, unsigned int flags,
...
//       .def(py::init([](const std::vector<std::string>& args, std::string product,
//                        std::string version) {
//         std::vector<char *> c_args;
//         for (const auto &arg : args) {
//           c_args.push_back(const_cast<char *>(arg.c_str()));
...
//         char **argv = c_args.data();
//         int argc = static_cast<int>(args.size());
//         return new s_vpi_vlog_info{argc, argv, const_cast<char*>(product.c_str()), const_cast<char*>(version.c_str())};
//       }))
//       .def(py::init<>())
...
//       .def_readwrite("version", &s_vpi_vlog_info::version);
  ...
  vpi.def("vpi_mcd_open", &vpi_mcd_open, py::arg("fileName"),
          "Open a memory content description file.");

  vpi.def("vpi_mcd_close", &vpi_mcd_close, py::arg("mcd"),
          "Close a memory content description file.");

  vpi.def("vpi_mcd_name", &vpi_mcd_name, py::arg("cd"),
          "Get the name of a memory content description file.");

  vpi.def("vpi_mcd_printf", &vpi_mcd_printf_wrap,
          "Print to a memory content description file.");
  ...
  vpi.def("vpi_vprintf", &vpi_vprintf, py::arg("format"), py::arg("args"),
          "Print with variable arguments.");

  vpi.def(
      "vpi_mcd_vprintf", &vpi_mcd_vprintf, py::arg("mcd"), py::arg("format"),
      py::arg("args"),
      "Print to a memory content description file with variable arguments.");

  vpi.def("vpi_flush", &vpi_flush, "Flush output.");

  vpi.def("vpi_mcd_flush", &vpi_mcd_flush, py::arg("mcd"),
          "Flush a memory content description file.");
