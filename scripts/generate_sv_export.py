...
def get_sv_export(source_dir):
    filename = source_dir / "src" / "svuvm" / "python_bridge_pkg.sv"
    tree = pyslang.syntax.SyntaxTree.fromFile(str(filename))
    root = tree.root
...
def get_cheader(source_dir):
    ...
    (source_dir / "inc" / "export_dpi.h").write_text("\n".join(output_lines))

def main():
    if len(sys.argv) < 2:
        print("Usage: generate_sv_export.py <source_dir>")
        sys.exit(1)

    source_dir = Path(sys.argv[1])
    get_sv_export(source_dir)
    get_cheader(source_dir)
    print(f"Generated DPI exports for {source_dir}")
