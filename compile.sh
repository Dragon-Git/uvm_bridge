...
verilator --binary +define+NOUVM -top top $(python -m pysvuvm.config) -LDFLAGS "$(python3-config --ldflags --embed)"  $PWD/test/api.sv $PWD/uvm-verilator/src/uvm.sv +incdir+$PWD/uvm-verilator/src
./obj_dir/Vtop